...
public class CustomerOptionsSetController {
	...
	public String displayOptions(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale)
			throws Exception {
	...
	public String saveOptionSet(@Valid @ModelAttribute("optionSet") CustomerOptionSet optionSet, BindingResult result,
		...
		// get from DB
		// option =
		// customerOptionService.getById(optionSet.getPk().getCustomerOption().getId());
	...
	public @ResponseBody ResponseEntity<String> pageOptionsSet(HttpServletRequest request,
		...
		try {
			...
			if (optionSet != null && optionSet.size() > 0) {
				...
				for (CustomerOptionSet optSet : optionSet) {
					...
					// optSet.getPk().getCustomerOptionValue();
					...
					CustomerOptionValueDescription valueDescription = customerOptionValue.getDescriptionsSettoList()
							.get(0);
	...
	public @ResponseBody ResponseEntity<String> deleteOptionSet(HttpServletRequest request,
		...
		try {
			...
			// {
			if (entity == null
					|| entity.getCustomerOption().getMerchantStore().getId().intValue() != store.getId().intValue()) {
