...
public class OrderFacadeImpl implements OrderFacade {
	...
	private Order processOrderModel(ShopOrder order, Customer customer, Transaction transaction, MerchantStore store,
		...
		try {
			...
			modelOrder.setLocale(LocaleUtils.getLocale(store));// set the store locale based on the country for order $
																// formatting
			...
			if (order.getPayment() != null && order.getPayment().get("paymentToken") != null) {// set any tokenization
																								// payment token
	...
	public Order processOrder(PersistableOrderApi order, Customer customer, MerchantStore store, Language language,
		...
		try {
			...
			// requires Order Totals, this needs recalculation and then compare total with
			// the amount sent as part
			// of process order request. If totals does not match, an error should be
			// thrown.
