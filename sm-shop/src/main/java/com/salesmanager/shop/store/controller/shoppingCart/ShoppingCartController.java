...
public class ShoppingCartController extends AbstractController {
	...
	public @ResponseBody ShoppingCartData addShoppingCartItem(@RequestBody final ShoppingCartItem item,
		...
		// if a cart exist in the database add the item to the cart and put cart in
		// httpsession and save to the database
		// else a cart does not exist in the database, create a new one, set the
		// customer id, set the cart in the httpsession
		// else a cart exist in the httpsession, add item to httpsession cart and save
		// to the database
		...
		/**
		 * Tested with the following : what if you add item in the shopping cart as an
		 * anonymous user later on you log in to process with checkout but the system
		 * retrieves a previous shopping cart saved in the database for that customer in
		 * that case we need to synchronize both carts and the original one (the one
		 * with the customer id) supercedes the current cart in session the system will
		 * have to deal with the original one and remove the latest
		 */
	...
	String removeShoppingCartItem(final Long lineItemId, final HttpServletRequest request,
		...
		// ** need to check if the item has property, similar items may exist but with
		// different properties
		// String attributes = request.getParameter("attribute");//attributes id are
		// sent as 1|2|5|
