...
public class ContactController extends AbstractController {
	...
	public String displayContact(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale)
			throws Exception {
		...
		/** template **/
		StringBuilder template = new StringBuilder().append(ControllerConstants.Tiles.Content.contactus).append(".")
	...
	public @ResponseBody String sendEmail(@ModelAttribute(value = "contact") ContactForm contact,
		...
		try {
			...
			// FieldError("captchaResponseField","captchaResponseField",messages.getMessage("NotEmpty.contact.captchaResponseField",
			// locale));
			...
			// reCaptcha.setPublicKey( coreConfiguration.getProperty(
			// ApplicationConstants.RECAPTCHA_PUBLIC_KEY));
			// reCaptcha.setPrivateKey( coreConfiguration.getProperty(
			// ApplicationConstants.RECAPTCHA_PRIVATE_KEY ) );
			 ...
			 * request.getRemoteAddr(), contact.getCaptchaChallengeField(),
			 * contact.getCaptchaResponseField() ); if ( !reCaptchaResponse.isValid() ) {
			 * LOGGER.debug( "Captcha response does not matched" ); FieldError error = new
			 * FieldError("captchaChallengeField","captchaChallengeField",messages.
			 * getMessage("validaion.recaptcha.not.matched", locale));
			 * bindingResult.addError(error); }
			 * 
			 * }
			 */
			...
			emailTemplatesUtils.sendContactEmail(contact, store, LocaleUtils.getLocale(store.getDefaultLanguage()),
