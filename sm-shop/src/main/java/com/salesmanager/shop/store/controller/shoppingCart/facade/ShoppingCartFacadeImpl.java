...
public class ShoppingCartFacadeImpl implements ShoppingCartFacade {
	...
	public ShoppingCartData getShoppingCartData(final Customer customer, final MerchantStore store,
		 ...
		 * List<ShoppingCartItem> availables = new ArrayList<ShoppingCartItem>(); //Take
		 * out items no more available List<ShoppingCartItem> items =
		 * shoppingCartData.getShoppingCartItems(); for(ShoppingCartItem item : items) {
		 * String code = item.getProductCode(); Product p
		 * =productService.getByCode(code, language); if(!p.isAvailable()) {
		 * unavailables.add(item); } else { availables.add(item); }
		 * 
	...
	@Override
	public void removeShoppingCartItem(String cartCode, Long productId, MerchantStore merchant, Language language)
			throws Exception {
	...
	private ReadableShoppingCart modifyCart(ShoppingCart cartModel, PersistableShoppingCartItem item,
		...
		// com.salesmanager.core.model.shoppingcart.ShoppingCartItem affectedItem =
		// null;
