...
public class FilePathUtils {
	...
	public String buildAdminDownloadProductFilePath(MerchantStore store, DigitalProduct digitalProduct) {
		return new StringBuilder().append(Constants.ADMIN_URI).append(Constants.FILES_URI).append(DOWNLOADS)
				.append(store.getCode()).append(Constants.SLASH).append(digitalProduct.getProductFileName()).toString();
	}
	...
	public String buildOrderDownloadProductFilePath(MerchantStore store, ReadableOrderProductDownload digitalProduct,
		...
		return new StringBuilder().append(Constants.SHOP_URI).append(Constants.ORDER_DOWNLOAD_URI)
				.append(Constants.SLASH).append(orderId).append(Constants.SLASH).append(digitalProduct.getId())
				.append(Constants.URL_EXTENSION).toString();
	}
	...
	public String buildStaticFileAbsolutePath(MerchantStore store, String fileName) {
		...
		if (!StringUtils.isBlank(imageUtils.getBasePath())
			...
			// Map<String,String> configurations = (Map<String,
			// String>)request.getSession().getAttribute(Constants.STORE_CONFIGURATION);
			...
			storePath.append(scheme).append("://")
					...
					.append(store.getDomainName()).append(coreConfiguration.getProperty("CONTEXT_PATH"));
	...
	public String buildStoreUri(MerchantStore store, String contextPath) {
		...
		resourcePath.append(scheme).append(DOUBLE_SLASH).append(domainName).append(path);
	...
	private String buildBaseUrl(HttpServletRequest request, MerchantStore store) {
		...
		resourcePath.append(scheme).append(DOUBLE_SLASH).append(domainName).append(contextPath);
	...
	public String buildAdminUri(MerchantStore store, HttpServletRequest request) {
		...
		resourcePath.append(baseUrl).append(Constants.ADMIN_URI);
	...
	public String buildCategoryUrl(MerchantStore store, String contextPath, String url) {
		...
		resourcePath.append(buildStoreUri(store, contextPath))
				...
				.append(Constants.CATEGORY_URI).append(Constants.SLASH).append(url).append(Constants.URL_EXTENSION);
	...
	public String buildProductUrl(MerchantStore store, String contextPath, String url) {
		StringBuilder resourcePath = new StringBuilder();
		resourcePath.append(buildStoreUri(store, contextPath)).append(Constants.SHOP_URI).append(Constants.PRODUCT_URI)
				.append(Constants.SLASH).append(url).append(Constants.URL_EXTENSION);
