 ...
 * https://www.google.com/recaptcha/api/siteverify Throws an exception if it
 * can't connect to reCaptcha returns true or false if validation has passed
 * 
 * @author carlsamson
 *
 */
...
public class CaptchaRequestUtils {
	...
	public boolean checkCaptcha(String gRecaptchaResponse) throws Exception {
		...
		/*
		 * NameValuePair[] data = { new NameValuePair("secret",
		 * configuration.getProperty(ApplicationConstants.RECAPTCHA_PRIVATE_KEY)), new
		 * NameValuePair("response", gRecaptchaResponse) };
		 */
