...
public class StoreFilter extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		try {
			...
			/**
			 * Get global objects Themes are built on a similar way displaying Header, Body
			 * and Footer Header and Footer are displayed on each page Some themes also
			 * contain side bars which may include similar emements
			 * 
			 * Elements from Header : - CMS links - Customer - Mini shopping cart - Store
			 * name / logo - Top categories - Search
			 * 
			 * Elements from Footer : - CMS links - Store address - Global payment
			 * information - Global shipping information
			 */

			// get from the cache first
			/**
			 * The cache for each object contains 2 objects, a Cache and a Missed-Cache Get
			 * objects from the cache If not null use those objects If null, get entry from
			 * missed-cache If missed-cache not null then nothing exist If missed-cache
			 * null, add missed-cache entry and load from the database If objects from
			 * database not null store in cache
			 */
			...
			/**
			 * SHOP configuration type Should contain - Different configuration flags -
			 * Google analytics - Facebook page - Twitter handle - Show customer login - ...
			 */
	...
	private void getContentPageNames(MerchantStore store, Language language, HttpServletRequest request)
		...
		/**
		 * CMS links Those links are implemented as pages (Content) ContentDescription
		 * will provide attributes name for the label to be displayed and seUrl for the
		 * friendly url page
		 */
	...
	private Map<String, List<ReadableCategory>> getCategories(MerchantStore store, Language language) throws Exception {
		...
		/**
		 * returns categories with required depth, 0 = root category, 1 = root + 1 layer
		 * child ...)
		 **/
		...
		if (categories != null && categories.size() > 0) {
			...
			for (Category category : categories) {
				if (category.isVisible()) {
					...
					// readableCategoryPopulator.populate(category, readableCategory, store,
					// language);
