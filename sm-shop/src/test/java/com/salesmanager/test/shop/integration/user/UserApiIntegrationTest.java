...
public class UserApiIntegrationTest extends ServicesTestSupport {
	...
	public void getUser() throws Exception {
		...
		final ResponseEntity<ReadableUser> response = testRestTemplate.exchange(
				String.format("/api/v1/private/users/" + DEFAULT_USER_ID), HttpMethod.GET, httpEntity,
				ReadableUser.class);
	...
	public void createUserChangePassword() throws Exception {
		...
		final ResponseEntity<ReadableUser> response = testRestTemplate.exchange(String.format("/api/v1/private/user/"),
				HttpMethod.POST, persistableUser, ReadableUser.class);
		if (response.getStatusCode() != HttpStatus.OK) {
			throw new Exception(response.toString());
		} else {
		...
		final ResponseEntity<Void> changePassword = testRestTemplate.exchange(
				String.format("/api/v1/private/user/" + user.getId() + "/password"), HttpMethod.PUT,
				changePasswordEntity, Void.class);
