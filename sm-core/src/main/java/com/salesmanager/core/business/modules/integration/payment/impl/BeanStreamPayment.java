...
public class BeanStreamPayment implements PaymentModule {
	...
	public Transaction capture(MerchantStore store, Customer customer, Order order, Transaction capturableTransaction,
		...
		try {
			...
			Transaction response = this.sendTransaction(null, store, messageString.toString(), "PAC",
					TransactionType.CAPTURE, PaymentType.CREDITCARD, order.getTotal(), configuration, module);
	...
	public Transaction refund(boolean partial, MerchantStore store, Transaction transaction, Order order,
		...
		try {
			...
			Transaction response = this.sendTransaction(null, store, messageString.toString(), "R",
					TransactionType.REFUND, PaymentType.CREDITCARD, amount, configuration, module);
	...
	private Transaction sendTransaction(String orderNumber, MerchantStore store, String transaction,
		...
		try {
			 ...
			 * Languedoc &ordCity=Boucherville &ordProvince=QC &ordPostalCode=J4B8J9
	...
	private Transaction processTransaction(MerchantStore store, Customer customer, TransactionType type,
		...
		try {
			 ...
			 * transaction settings in your Beanstream merchant member area to allow for
			 * this transaction type. - Log in to the Beanstream online member area at
